...
const risecheckoutForm = {
	init: function () {
		...
		[ 'input', 'paste' ].forEach( ( type ) => {
			this.form
				.querySelector( '[name=postcode]' )
				.addEventListener( type, this.updatePostcode );
		} );
	},
	...
	setupInfoFields: function () {
		...
		step.querySelectorAll( '[data-info], [data-info-prefix]' ).forEach(
			( info ) => {
				...
				if ( ! document.getElementById( `info-${ infoId }` ) ) {
					const label =
						info.dataset.infoLabel ||
						step
							.querySelector( `label[for=${ info.id }` )
							.textContent.trim() ||
						'';
					const content = info.hasAttribute( 'data-info-prefix' )
						? `${ label } `
						: '';
					const infoP = this.createElement( 'p', infoId, content, {
						id: `info-${ infoId }`,
						'data-label': label,
					} );
	...
	updateInfoText: function ( info ) {
		...
		if ( info.dataset.info ) {
			infoValue = Array.from(
				form.querySelectorAll( `[data-info=${ infoId }]` )
			)
				.map( ( item ) => item.value )
				.join( ' ' )
				.trim();
		...
		if ( info.hasAttribute( 'data-info-prefix' ) ) {
			document.querySelector( `#info-${ infoId } span` ).textContent =
				infoValue;
		} else {
			document.getElementById( `info-${ infoId }` ).textContent =
				infoValue;
		}
	},
	...
	cleanFormData: function ( formData ) {
		...
		for ( let [ key, value ] of formData.entries() ) {
			...
			if (
				maybePrefix &&
				maybePrefix.classList.contains( 'input-group-text' )
			) {
	...
	submit: function ( event ) {
		...
		step.querySelectorAll( '.form-control' ).forEach( ( field ) =>
			field.setAttribute( 'disabled', '' )
		);
		step.querySelector( '.btn-send' ).classList.add( 'sending' );

		fetch(
			this.params.wcAjaxUrl.replace(
				'%%endpoint%%',
				'risecheckout_customer'
			),
			{
				method: 'POST',
				body,
	...
	fillAddress: function ( postcode ) {
		...
		if ( postcode.length !== 8 ) {
			...
			fetch(
				params.wcAjaxUrl.replace(
					'%%endpoint%%',
					'risecheckout_postcode_br'
				),
				{
					method: 'POST',
					body: postcode,
				...
				.then( ( response ) => {
					...
					if ( response.success ) {
						...
						const fields = step.querySelectorAll(
							'.address-field .form-control:not([name=receiver])'
						);
						...
						Object.keys( data ).forEach( ( name ) => {
							...
							let place = this.form.querySelector(
								`[name=${ name }]`
							);
							console.log( place );
							if ( ! place ) {
								place = this.form.querySelector(
									`[data-info="${ name }"]`
								);
								console.log( place );
	...
	updatePostcode: function ( event ) {
		const field = this.form.querySelector( '[name=postcode]' );
		if (
			! field.hasAttribute( 'data-mask' ) ||
			field.dataset.mask !== 'postcode-br'
		) {
			return;
		}
		let postcode;
		if ( event.target ) {
			...
			postcode = ( event.clipboardData || window.clipboardData ).getData(
				'text'
			);
		}
	...
	appendOverlaySpinner: function () {
		...
		const overlay = this.createElement(
			'div',
			'overlay-spinner overlay-spinner-box'
		);
		overlay.appendChild(
			this.createElement( 'div', 'spinner spinner-grey' )
		);
	...
	setupLoadingFields: function () {
		this.form.querySelectorAll( '[data-loading]' ).forEach( ( field ) => {
			...
			const spinnerDiv = this.createElement(
				'div',
				'spinner spinner-grey spinner-form'
			);
	...
	setupCityStateInfo: function () {
		...
		const cityStateInfoWrapper = this.createElement(
			'div',
			classList.join( ' ' ).replace( 'col-8', 'col-5' )
		);

		const cityStateInfoDiv = this.createElement(
			'div',
			'city-state-info-text',
			'',
			{ id: 'cityStateInfo' }
		);
		...
		fields.forEach( ( field, index ) => {
			if ( index > 0 ) {
				cityStateInfoDiv.appendChild(
					document.createTextNode( ' / ' )
				);
			}
			const span = this.createElement( 'span', field.id, field.value, {
				'data-info': field.id,
			} );
		...
		const cityStateInfoParts = this.form.querySelectorAll(
			'#cityStateInfo span'
		);
