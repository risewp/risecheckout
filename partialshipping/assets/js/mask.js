const risecheckoutMask = {
	...
	formatCpf: function ( value ) {
		...
		if ( value.length > 6 ) {
			formatted =
				formatted.substring( 0, 7 ) + '.' + formatted.substring( 7 );
		}
		if ( value.length > 9 ) {
			formatted =
				formatted.substring( 0, 11 ) + '-' + formatted.substring( 11 );
		}
		return formatted;
	},
			...
			if ( 'phone-br' === mask ) {
				...
				if ( postcodeField ) {
					postcodeField.addEventListener( 'input', ( e ) => {
						e.target.value = this.formatPostcodeBr(
							e.target.value
						);
					} );
					...
					postcodeField.addEventListener( 'paste', ( e ) => {
						...
						postcodeField.value =
							this.formatPostcodeBr( pastedData );
					} );
