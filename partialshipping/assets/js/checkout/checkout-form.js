...
const wc_checkout_form = {
	...
	init: function () {
		...
		this.$checkout_form.addEventListener(
			'update',
		...
		this.$checkout_form.addEventListener( 'change', ( event ) => {
			if (
				event.target.matches(
					'select.shipping_method, input[name^="shipping_method"], #ship-to-different-address input'
				)
			) {
		...
		document
			.querySelectorAll( '#ship-to-different-address input' )
			.forEach( ( input ) =>
		...
		if ( wc_checkout_params.option_guest_checkout === 'yes' ) {
			const createAccountInput = document.querySelector(
				'input#createaccount'
			);
			if ( createAccountInput ) {
				createAccountInput.addEventListener(
					'change',
					this.toggle_create_account
				);
	...
	init_payment_methods: function () {
		...
		const checkedPaymentMethod = Array.from( paymentMethods ).find(
	...
	get_payment_method: function () {
		return this.$checkout_form.querySelector(
	...
	trigger_update_checkout: function ( event ) {
		...
		document.body.dispatchEvent(
			new CustomEvent( 'update_checkout', {
				detail: { current_target: event ? event.target : null },
			} )
		);
	},
